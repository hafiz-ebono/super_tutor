...
**Current focus:** v6.0 Document Upload — Phase 13 (Frontend Upload UI)
...
**12-01 decisions:**
- source field added directly after session_type in GET response dict for logical grouping
- notes_step source persistence uses if-guard (only writes when non-empty) to stay backward compatible with URL/topic/paste sessions

**12-02 decisions:**
- Pre-stream validation: PDF check, size check, and extract_document all run before SSE opens; scanned PDFs return HTTP 422, not SSE error event
- source param added to run_workflow_background with default '' for backward compatibility
- _get_traces_db imported from session_workflow.py (not sessions.py) in upload.py to prevent circular import

**12-03 decisions:**
- SSE happy path test mocks extract_document at app.routers.upload import level — avoids constructing real pypdf-parseable PDF binary in tests
- client.stream() used for SSE test, client.post() for pre-stream HTTP error tests — consistent with endpoint behavior
- SC4 verified via combined-router TestClient mounting both sessions_router and upload_router at /sessions prefix
