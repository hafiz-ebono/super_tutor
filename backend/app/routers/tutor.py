...
from app.agents.tutor_team import build_tutor_team, is_rate_limit_error, TUTOR_TOKEN_EVENTS, TUTOR_ERROR_EVENT, TUTOR_COMPLETED_EVENT
    ...
    async def _stream_team(active_team, message: str) -> AsyncGenerator[dict, None]:
        ...
        async for chunk in active_team.arun(message, stream=True, session_id=tutor_session_id):
            if chunk.event == TUTOR_ERROR_EVENT:
                error_chunk = chunk
                break
            if chunk.event == TUTOR_COMPLETED_EVENT:
                member_responses = getattr(chunk, "member_responses", None) or []
                names = [
                    getattr(r, "agent_name", None) or getattr(r, "team_name", None)
                    for r in member_responses
                    if getattr(r, "agent_name", None) or getattr(r, "team_name", None)
                ]
                logger.info(
                    "Tutor delegation — session_id=%s routed_to=%s",
                    session_id,
